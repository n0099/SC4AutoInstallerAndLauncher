...
Module ModuleInstallModule
    ...
    Public Function InstallDAEMONTools(ByVal InstallDir As String) As InstallResult.Result
        Try
            ...
            '以管理员权限运行DAEMON Tools Lite安装程序以便静默安装DAEMON Tools Lite并等待其安装完成
    ...
    Public Function InstallSC4(ByVal SC4InstallDir As String, ByVal DAEMONInstallDir As String, ByVal InstallType As InstallOptions.SC4InstallType) As InstallResult.Result
        Try
            My.Computer.FileSystem.CopyFile(Application.ExecutablePath, SC4InstallDir & "\Setup.exe", True) '将安装程序复制到SC4InstallDir下
            ...
            If InstallType = InstallOptions.SC4InstallType.ISO Then
                ...
                Do Until My.Computer.FileSystem.FileExists("X:\AutoRun.exe") '将CD1虚拟光驱镜像文件加载到X盘符上
                    '以管理员权限启动DAEMON Tools Lite以便加载CD1并等待其加载完成
                ...
                Do Until My.Computer.FileSystem.FileExists("Y:\RunGame.exe") '将CD2虚拟光驱镜像文件加载到Y盘符上
                    '以管理员权限启动DAEMON Tools Lite以便加载CD2并等待其加载完成
                ...
                If My.Computer.FileSystem.FileExists(TempFolder & "\AutoRun.exe") = False Or My.Computer.FileSystem.FileExists(TempFolder & "\AutoRunGUI.dll") = False Then
                    ...
                    Process.Start(New ProcessStartInfo With {.FileName = "regsvr32.exe", .Arguments = "/s """ & TempFolder & "\AutoRunGUI.dll""", .Verb = "runas"}).WaitForExit() '以管理器权限运行regsvr32.exe以便注册AutoRunGUI.dll文件
                End If

                Dim SetupProcess As Process '声明一个System.Diagnostics.Process类的实例以便记录AutoRun.exe进程的退出代码
                SetupProcess = Process.Start(New ProcessStartInfo With {.FileName = TempFolder & "\AutoRun.exe", .Arguments = "-restart -dir X:\", .Verb = "runas"}) '以管理员权限运行临时文件目录下的安装程序

                Do Until FindWindow("#32770", "SimCity 4 Deluxe") <> Nothing : Loop '等待主窗口出现
                ...
                Threading.Thread.Sleep(100) : Do Until Process.GetProcessesByName("SimCity 4 Deluxe_Code").Length <> 0 And FindWindow("#32770", "SimCity 4 Deluxe") <> Nothing : Loop '等待SimCity 4 Deluxe_Code.exe进程和序列号窗口出现
                ...
                Threading.Thread.Sleep(100) : Do Until FindWindow("#32770", "SimCity 4 Deluxe") <> Nothing : Loop '等待安装路径对话框出现
                ...
                Threading.Thread.Sleep(100) : Do Until Process.GetProcessesByName("SimCity 4 Deluxe_eReg").Length <> 0 And FindWindow("#32770", "Electronic Registration") <> Nothing : Loop '等待SimCity 4 Deluxe_eReg.exe进程和注册EA账号对话框出现
                ...
                Threading.Thread.Sleep(100) : Do Until FindWindow("#32770", "") <> Nothing : Loop '等待确定不注册EA账号对话框出现
                ...
                '将GOG版模拟城市4的Graphics Rules.sgr文件解压到SC4InstallDir下
                '以管理员权限运行7za.exe并隐藏进程窗口以便将Data\SC4\NoInstall.7z压缩包的Graphics Rules.sgr文件解压到SC4InstallDir下替换源文件并等待其完成解压缩
                _7zaProcess = Process.Start(New ProcessStartInfo With {.FileName = "Data\7za.exe", .Arguments = "x Data\SC4\NoInstall.7z -aoa ""Graphics Rules.sgr"" -o""" & SC4InstallDir & """", .Verb = "runas", .WindowStyle = ProcessWindowStyle.Hidden})
                _7zaProcess.WaitForExit()
                Return IIf(My.Computer.FileSystem.FileExists(SC4InstallDir & "\Apps\SimCity 4.exe" OrElse _7zaProcess.ExitCode = 0), InstallResult.Result.Success, InstallResult.Result.Fail)
            ElseIf InstallType = InstallOptions.SC4InstallType.NoInstall Then
                ...
                '以管理员权限运行7za.exe并隐藏进程窗口以便将Data\SC4\NoInstall.7z压缩包的所有文件解压到SC4InstallDir下替换源文件并等待其完成解压缩
                _7zaProcess = Process.Start(New ProcessStartInfo With {.FileName = "Data\7za.exe", .Arguments = "x Data\SC4\NoInstall.7z -aoa -o""" & SC4InstallDir & """", .Verb = "runas", .WindowStyle = ProcessWindowStyle.Hidden})
                _7zaProcess.WaitForExit()
    ...
    Public Function Install638Patch(ByVal InstallDir As String, ByVal IsUninstall As Boolean) As InstallResult.Result
        Try
            ...
            If IsUninstall = False Then
                ...
                '以管理员权限运行7za.exe并隐藏进程窗口以便将Data\Patch\638.7z压缩包的所有文件解压到InstallDir下替换源文件并等待其完成解压缩
                _7zaProcess = Process.Start(New ProcessStartInfo With {.FileName = "Data\7za.exe", .Arguments = "x Data\Patch\638.7z -aoa -o""" & InstallDir & """", .Verb = "runas", .WindowStyle = ProcessWindowStyle.Hidden})
                _7zaProcess.WaitForExit()
            Else
                ...
                For Each i As String In Files : Do Until IsFileUsing(i) = False : Loop : Next '确保没有进程正在使用Files字符串数组所存储的文件
                '以管理员权限运行7za.exe并隐藏进程窗口以便将Data\SC4\NoInstall.7z压缩包的Apps\SimCity 4.exe和SimCity_1到5.dat文件解压到InstallDir下替换源文件并等待其完成解压缩
                _7zaProcess = Process.Start(New ProcessStartInfo With {.FileName = "Data\7za.exe", .Arguments = "x Data\SC4\NoInstall.7z ""Apps\SimCity 4.exe"" ""SimCity_*.dat"" -aoa -o""" & InstallDir & """", .Verb = "runas", .WindowStyle = ProcessWindowStyle.Hidden})
                _7zaProcess.WaitForExit()
            End If
            Return IIf(_7zaProcess.ExitCode = 0, InstallResult.Result.Success, InstallResult.Result.Fail)
        Catch
    ...
    Public Function Install640Patch(ByVal InstallDir As String, ByVal IsUninstall As Boolean) As InstallResult.Result
        Try
            ...
            If IsUninstall = False Then
                ...
                For Each i As String In Files : Do Until IsFileUsing(i) = False : Loop : Next '确保没有进程正在使用Files字符串数组所存储的文件
                '以管理员权限运行7za.exe并隐藏进程窗口以便将Data\Patch\640.7z压缩包的所有文件解压到InstallDir下替换源文件并等待其完成解压缩
                _7zaProcess = Process.Start(New ProcessStartInfo With {.FileName = "Data\7za.exe", .Arguments = "x Data\Patch\640.7z -aoa -o""" & InstallDir & """", .Verb = "runas", .WindowStyle = ProcessWindowStyle.Hidden})
                _7zaProcess.WaitForExit()
            Else
    ...
    Public Function Install641Patch(ByVal InstallDir As String, ByVal IsUninstall As Boolean) As InstallResult.Result
        Try
            If IsUninstall = False Then
                Do Until IsFileUsing(InstallDir & "\Apps\SimCity 4.exe") = False : Loop '确保没有进程正在使用InstallDir\Apps\SimCity_4.exe文件
                My.Computer.FileSystem.CopyFile("Data\Patch\SimCity 4 641.exe", InstallDir & "\Apps\SimCity 4.exe", True) '将Data\Patch\SimCity 4 641.exe复制到InstallDir\Apps目录下并重命名为SimCity 4.exe替换源文件
    ...
    Public Function Install4GBPatch(ByVal InstallDir As String, ByVal IsUninstall As Boolean, ByVal InstallOptions As InstallOptions) As InstallResult.Result
        With InstallOptions
            Try
                If IsUninstall = False Then
                    Do Until IsFileUsing(InstallDir & "\Apps\SimCity 4.exe") = False : Loop '确保没有进程正在使用InstallDir\Apps\SimCity_4.exe文件
                    Process.Start(New ProcessStartInfo With {.FileName = "Data\Patch\4GB.exe", .Arguments = """" & InstallDir & "\Apps\SimCity 4.exe""", .Verb = "runas"}).WaitForExit() '以管理员权限运行4GB.exe以便安装4GB补丁并等待其完成安装
                    '声明一个用于计算MD5值的System.Security.Cryptography.MD5CryptoServiceProvider类实例和一个用于存储InstallDir\Apps\SimCity 4.exe文件的MD5值的字符串变量
                    Dim MD5CSP As New Security.Cryptography.MD5CryptoServiceProvider, MD5 As String
                    MD5 = BitConverter.ToString(MD5CSP.ComputeHash(New IO.FileStream(InstallDir & "\Apps\SimCity 4.exe", IO.FileMode.Open))).Replace("-", "") '将InstallDir\Apps\SimCity 4.exe文件的MD5值存储到变量MD5里
                    '验证InstallDir\Apps\SimCity 4.exe文件的MD5值来确定是否成功安装4GB补丁
                ...
                Else
                    ...
                    If .Install638Patch = True And .Install640Patch = False And .Install641Patch = False Then
                        Do Until IsFileUsing("Data\Patch\638.7z") = False And IsFileUsing(InstallDir & "\Apps\SimCity 4.exe") = False : Loop '确保没有进程正在使用Data\Patch\638.7z和InstallDir\Apps\SimCity 4.exe文件
                        '以管理员权限运行7za.exe并隐藏进程窗口以便将Data\Patch\638.7z压缩包的Apps\SimCity 4.exe文件解压到InstallDir下替换源文件并等待其完成解压缩
                        _7zaProcess = Process.Start(New ProcessStartInfo With {.FileName = "Data\7za.exe", .Arguments = "x Data\Patch\638.7z ""Apps\SimCity 4.exe"" -aoa -o""" & InstallDir & """", .Verb = "runas", .WindowStyle = ProcessWindowStyle.Hidden})
                        _7zaProcess.WaitForExit()
                        Return IIf(_7zaProcess.ExitCode = 0, InstallResult.Result.Success, InstallResult.Result.Fail)
                    ElseIf .Install638Patch = True And .Install640Patch = True And .Install641Patch = False Then
                        ...
                        '以管理员权限运行7za.exe并隐藏进程窗口以便将Data\Patch\640.7z压缩包的Apps\SimCity 4.exe文件解压到InstallDir下替换源文件并等待其完成解压缩
                        _7zaProcess = Process.Start(New ProcessStartInfo With {.FileName = "Data\7za.exe", .Arguments = "x Data\Patch\640.7z ""Apps\SimCity 4.exe"" -aoa -o""" & InstallDir & """", .Verb = "runas", .WindowStyle = ProcessWindowStyle.Hidden})
                        _7zaProcess.WaitForExit()
                        Return IIf(_7zaProcess.ExitCode = 0, InstallResult.Result.Success, InstallResult.Result.Fail)
                    ...
                    ElseIf .Install638Patch = False And .Install640Patch = False And .Install641Patch = False Then
                        Do Until IsFileUsing("Data\SC4\NoInstall.7z") = False And IsFileUsing(InstallDir & "\Apps\SimCity 4.exe") = False : Loop '确保没有进程正在使用Data\SC4\NoInstall.7z和InstallDir\Apps\SimCity 4.exe文件
                        '以管理员权限运行7za.exe并隐藏进程窗口以便将Data\SC4\NoInstall.7z压缩包的Apps\SimCity 4.exe文件解压到InstallDir下替换源文件并等待其完成解压缩
    ...
    Public Function InstallNoCDPatch(ByVal InstallDir As String, ByVal IsUninstall As Boolean) As InstallResult.Result
        Try
            If IsUninstall = False Then
                Do Until IsFileUsing(InstallDir & "\Apps\SimCity 4.exe") = False : Loop '确保没有进程正在使用InstallDir\Apps\SimCity_4.exe文件
                My.Computer.FileSystem.CopyFile("Data\Patch\SimCity 4 NoCD.exe", InstallDir & "\Apps\SimCity 4.exe", True) '将Data\Patch\SimCity 4 NoCD.exe复制到InstallDir\Apps目录下并重命名为SimCity 4.exe替换源文件
                Return IIf(My.Computer.FileSystem.GetFileInfo(InstallDir & "\Apps\SimCity 4.exe").Length = 7524352, InstallResult.Result.Success, InstallResult.Result.Fail)
            Else
    ...
    Public Function InstallSC4Launcher(ByVal InstallDir As String, ByVal IsUninstall As Boolean) As InstallResult.Result
        Try
            If IsUninstall = False Then
                My.Computer.FileSystem.CopyFile("Data\SC4Launcher.exe", InstallDir & "\SC4Launcher.exe", True) '将Data\SC4Launcher.exe复制到InstallDir下替换源文件
            ...
            Else
                Do Until IsFileUsing(InstallDir & "\SC4Launcher.exe") = False : Loop '确保没有进程正在使用InstallDir\SC4Launcher.exe文件
                My.Computer.FileSystem.DeleteFile(InstallDir & "\SC4Launcher.exe") '删除InstallDir\SC4Launcher.exe文件
    ...
    ''' <param name="InstallDir">指定安装路径</param>
    ''' <param name="LanguagePatch">InstallOptions.Language的值之一，指定要安装的语言补丁的语言</param>
    ''' <returns>InstallResult.Result的值之一，如果安装或卸载成功，则为InstallResult.Result.Success；否则为InstallResult.Result.Fail</returns>
            ...
            Try
                ...
                Select Case LanguagePatch
                    Case InstallOptions.Language.TraditionalChinese
                        My.Computer.FileSystem.CopyDirectory("Data\Patch\Language\TChinese", InstallDir & "\TChinese", True) '将Data\Patch\Language\TChinese文件夹复制到InstallDir下替换源文件
                    ...
                    Case InstallOptions.Language.SimplifiedChinese
                        My.Computer.FileSystem.CopyDirectory("Data\Patch\Language\SChinese", InstallDir & "\SChinese", True) '将Data\Patch\Language\SChinese文件夹复制到InstallDir下替换源文件
                    ...
                    Case Else
                        My.Computer.FileSystem.CopyDirectory("Data\Patch\Language\English", InstallDir & "\English", True) '将Data\Patch\Language\English文件夹复制到InstallDir下替换源文件
    ...
    ''' <remarks>如果游戏安装目录下有名为SC4Launcher.exe的程序，则快捷方式会导向该文件</remarks>

Public Class frmOptions
            ...
            Select Case e.Node.Name
                ...
                Case "模拟城市4 豪华版 硬盘版"
                    If GetNodeChecked(e.Node.Name) = NodeCheckedState.radiounchecked Then
                        ...
                        lblNeedsDiskSpace.Text = "安装目录至少需要 " & .GetNeedsDiskSpace("GB") & "GB 的硬盘空间"
                    End If
                Case "638补丁", "640补丁", "4GB补丁", "模拟城市4 启动器"
                    ...
                    lblNeedsDiskSpace.Text = "安装目录至少需要 " & .GetNeedsDiskSpace("GB") & "GB 的硬盘空间"
                Case "638补丁"
                    If GetNodeChecked(e.Node.Name) = NodeCheckedState.checked Then .IsInstall638Patch = False
                    If GetNodeChecked(e.Node.Name) = NodeCheckedState.unchecked Then .IsInstall638Patch = True
                Case "640补丁"
                    If GetNodeChecked(e.Node.Name) = NodeCheckedState.checked Then .IsInstall640Patch = False
                    If GetNodeChecked(e.Node.Name) = NodeCheckedState.unchecked Then .IsInstall640Patch = True
                Case "4GB补丁"
                    If GetNodeChecked(e.Node.Name) = NodeCheckedState.checked Then .IsInstall4GBPatch = False
                    If GetNodeChecked(e.Node.Name) = NodeCheckedState.unchecked Then .IsInstall4GBPatch = True
                Case "免CD补丁"
                    If GetNodeChecked(e.Node.Name) = NodeCheckedState.checked Then
                    ...
                    lblNeedsDiskSpace.Text = "安装目录至少需要 " & .GetNeedsDiskSpace("GB") & "GB 的硬盘空间"
                Case "模拟城市4 启动器"
                    If GetNodeChecked(e.Node.Name) = NodeCheckedState.checked Then .IsInstallSC4Launcher = False
                    If GetNodeChecked(e.Node.Name) = NodeCheckedState.unchecked Then .IsInstallSC4Launcher = True
                Case "繁体中文"
                    If GetNodeChecked(e.Node.Name) = NodeCheckedState.radiounchecked Then
                        SetNodeChecked(e.Node.Name, NodeCheckedState.radiochecked) : SetNodeChecked("简体中文", NodeCheckedState.radiounchecked)
                        SetNodeChecked("英文", NodeCheckedState.radiounchecked) : .LanguagePatch = InstallOptions.Language.TraditionalChinese
                        lblNeedsDiskSpace.Text = "安装目录至少需要 " & .GetNeedsDiskSpace("GB") & "GB 的硬盘空间"
                    End If
                Case "简体中文"
                    If GetNodeChecked(e.Node.Name) = NodeCheckedState.radiounchecked Then
                        SetNodeChecked(e.Node.Name, NodeCheckedState.radiochecked) : SetNodeChecked("繁体中文", NodeCheckedState.radiounchecked)
                        SetNodeChecked("英文", NodeCheckedState.radiounchecked) : .LanguagePatch = InstallOptions.Language.SimplifiedChinese
                        lblNeedsDiskSpace.Text = "安装目录至少需要 " & .GetNeedsDiskSpace("GB") & "GB 的硬盘空间"
                    End If
                Case "英文"
                    If GetNodeChecked(e.Node.Name) = NodeCheckedState.radiounchecked Then
    ...
    Private Sub tvwOptions_NodeMouseHover(sender As Object, e As TreeNodeMouseHoverEventArgs) Handles tvwOptions.NodeMouseHover
        With tvwOptions
            Select Case e.Node.Name
                Case "必选组件"
                    lblOptionsDetail.Text = "请点击组件名以查看组件详情。" : lblOptionsDiskSpace.Text = ""
                Case "模拟城市4 豪华版 镜像版"
                ...
                Case "DAEMON Tools Lite"
                    If .Nodes.Find("DAEMON Tools Lite", True).Length <> 0 Then
                ...
                Case "语言补丁"
                    lblOptionsDetail.Text = "安装语言补丁" & vbCrLf & vbCrLf & "如果不安装语言补丁" & vbCrLf & vbCrLf & "默认语言为英文" & vbCrLf & vbCrLf & "建议安装。"
                    lblOptionsDiskSpace.Text = ""
                Case "繁体中文"
                    lblOptionsDetail.Text = "安装繁体中文语言补丁" & vbCrLf & vbCrLf & "如果不安装语言补丁" & vbCrLf & vbCrLf & "默认语言为英文" & vbCrLf & vbCrLf & "建议安装。"
                    lblOptionsDiskSpace.Text = "此组件需要 0KB 硬盘空间"
                Case "简体中文"
                    lblOptionsDetail.Text = "安装简体中文语言补丁" & vbCrLf & vbCrLf & "如果不安装语言补丁" & vbCrLf & vbCrLf & "默认语言为英文" & vbCrLf & vbCrLf & "不建议安装。"
                    lblOptionsDiskSpace.Text = "此组件需要 " & Int(InstallOptions.LanguageSimplifiedChineseNeedsDiskSpace / 1024) & "KB 硬盘空间"
                Case "英文"
                    lblOptionsDetail.Text = "安装英文语言补丁" & vbCrLf & vbCrLf & "不建议安装。"
                    lblOptionsDiskSpace.Text = "此组件需要 0KB 硬盘空间"
                Case "附加任务"
                    lblOptionsDetail.Text = "请点击组件名以查看组件详情。" : lblOptionsDiskSpace.Text = ""
                Case "添加桌面图标"

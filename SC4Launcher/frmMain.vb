    ...
    Private Sub bgwCheckUpdate_DoWork(sender As Object, e As System.ComponentModel.DoWorkEventArgs) Handles bgwCheckUpdate.DoWork
        Try '检查是否有新版本可用
            If My.Computer.Network.IsAvailable AndAlso My.Computer.Network.Ping("n0099.sinaapp.com") Then '确认能否连接到更新服务器
                ...
                e.Result = UpdateInfoXML.GetElementsByTagName("Launcher")(0) '返回已下载更新信息的XmlDocument类实例
            Else
WebError:       MessageBox.Show("无法连接更新服务器" & vbCrLf & "请检查网络连接后重试", "错误", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            End If
        Catch : e.Result = "ExceptionCatched" : GoTo WebError '如果检查更新途中发生异常则跳转至WebError行
        End Try
    End Sub
            ...
            With My.Application
                If (LatestVersion.Split(".")(0) > .Info.Version.Major) OrElse (LatestVersion.Split(".")(1) > .Info.Version.Minor) OrElse (LatestVersion.Split(".")(2) > .Info.Version.Revision) Then '判断最新版本号是否大于当前版本号
                    If MessageBox.Show("检测到有新版本可用，是否下载更新程序？" & vbCrLf & "当前版本：" & .Info.Version.Major & "." & .Info.Version.Minor & "." & .Info.Version.Revision & vbCrLf &
                                       "更新说明：" & LauncherNode("UpdateDetail").InnerText, "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) = DialogResult.Yes Then '询问用户是否更新
                        Me.Hide() : frmSetting.Hide() '隐藏其他窗口
                        My.Computer.Network.DownloadFile(LauncherNode("DonwloadLink").InnerText, Application.StartupPath & "\LauncherUpdate.exe", "", "", True, 6000000, True) '从指定的下载地址下载更新程序
...
#End Region
    ...
    Private Sub frmMain_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        End With
        BackgroundImage = CType(My.Resources.ResourceManager.GetObject("SC4_" & New Random().Next(1, 10)), Image) '将主窗口的背景图片设置为资源文件里名为SC4_随机数（介于1到10之间）的图片
